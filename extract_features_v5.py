...
def extract_features(ecg_df, save=True, mode='train'): ## feature amount = 43
    ...
    for key, val in fields.items():
        features_df['mean_'+key] = features_df.apply(lambda x: np.mean(x[val]), axis=1)
        features_df['min_'+key] = features_df.apply(lambda x: np.min(x[val]), axis=1)
        features_df['max_'+key] = features_df.apply(lambda x: np.max(x[val]), axis=1)
        features_df['std_'+key] = features_df.apply(lambda x: np.std(x[val]), axis=1)
        features_df['median_'+key] = features_df.apply(lambda x: np.median(x[val]), axis=1)
