...
print("HELLO")
X_train = pd.read_csv('raw/X_train.csv').drop(['id'], axis=1).values
X_test = pd.read_csv('raw/X_test.csv').drop(['id'], axis=1).values
...
np.savetxt('feat_main/features_train_temp_only.csv', Train_features, delimiter=',', fmt='%s')
Test_features = features_from_raw_data(X_test)
np.savetxt('feat_main/features_test_temp_only.csv', Test_features, delimiter=',', fmt='%s')
